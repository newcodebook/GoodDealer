import { spawnSync } from "node:child_process";
import { createHash } from "node:crypto";
import {
  existsSync,
  lstatSync,
  mkdirSync,
  readFileSync,
  readlinkSync,
  writeFileSync,
} from "node:fs";
import { arch, platform, release } from "node:os";
import { isAbsolute, relative, resolve } from "node:path";
import { fileURLToPath } from "node:url";

import { repositoryMaterialDirty } from "./git-dirty-state.mjs";
import { platformCommand } from "./platform-command.mjs";
import {
  CLOUD_BOUNDARY_EVIDENCE_DEFAULTS,
  CLOUD_BOUNDARY_REQUIRED_INPUTS,
  cloudBoundaryReportPassesPolicy,
} from "./collect-cloud-boundary-report.mjs";

const root = resolve(import.meta.dirname, "..");
const supportedProfiles = new Set(["local", "native", "quality"]);
const supportedSlices = new Set([
  "account-gate",
  "backup",
  "cloud-boundary",
  "jobs",
  "sqlcipher",
]);
export const BACKUP_EVIDENCE_INPUT_PATHS = Object.freeze([
  "Cargo.lock",
  "Cargo.toml",
  "pnpm-lock.yaml",
  "rust-toolchain.toml",
  "crates/local-storage/Cargo.toml",
  "crates/local-storage/src/lib.rs",
  "crates/local-storage/src/backup/mod.rs",
  "crates/local-storage/src/backup/evidence.rs",
  "crates/local-storage/src/sqlcipher_fixture.rs",
  "crates/local-storage/examples/backup_evidence.rs",
  "scripts/collect-wp0-evidence.mjs",
  "scripts/backup-evidence-policy.test.mjs",
]);
const portableCrates = [
  "-p",
  "gooddealer-secure-host-core",
  "-p",
  "gooddealer-local-storage",
  "--all-targets",
];
const keyInputPaths = [
  ".gitattributes",
  ".gitignore",
  ".node-version",
  "package.json",
  "pnpm-lock.yaml",
  "Cargo.toml",
  "Cargo.lock",
  "crates/local-storage/Cargo.toml",
  "crates/local-storage/examples/sqlcipher_evidence.rs",
  "crates/local-storage/src/sqlcipher_fixture.rs",
  "rust-toolchain.toml",
  "scripts/collect-wp0-evidence.mjs",
  "scripts/git-dirty-state.mjs",
  "scripts/git-dirty-state.test.mjs",
  "scripts/platform-command.mjs",
  "scripts/platform-command.test.mjs",
  "scripts/tauri-command-policy.mjs",
  "scripts/tauri-command-policy.test.mjs",
  "apps/desktop/src-tauri/build.rs",
  "apps/desktop/src-tauri/src/main.rs",
  "apps/desktop/src-tauri/capabilities/local-app.json",
  ".github/dependabot.yml",
  ".github/workflows/quality.yml",
  ".github/workflows/native.yml",
  ".github/workflows/wp5-sqlcipher.yml",
  "crates/secure-host-core/Cargo.toml",
];
const accountGateKeyInputPaths = [
  "scripts/collect-account-gate-report.mjs",
  "packages/client-core/test/runtime-mode.test.ts",
  "packages/protocol/src/account/index.ts",
  "packages/protocol/src/devices/index.ts",
  "packages/protocol/src/devices/bootstrap-steps.ts",
  "packages/protocol/src/wire/canonical-codec.ts",
  "packages/protocol/src/workspace/sync-mutation.ts",
  "apps/cloud/src/modules/identity/index.ts",
  "apps/cloud/src/modules/licensing/index.ts",
  "apps/cloud/src/modules/devices/index.ts",
  "apps/cloud/src/modules/devices/bootstrap-fixture.ts",
  "packages/protocol/test/bootstrap-steps.test.ts",
  "packages/protocol/test/workspace-sync.test.ts",
  "apps/cloud/test/bootstrap-fixture.test.ts",
  ".github/workflows/wp2-account-gate.yml",
];
const cloudBoundaryKeyInputPaths = [...CLOUD_BOUNDARY_REQUIRED_INPUTS];
const sqlcipherRequiredChecks = [
  "databaseAndWalEncrypted",
  "rollbackJournalEncrypted",
  "wrongKeyRejected",
  "truncatedDatabaseRejected",
  "tamperedDatabaseRejected",
  "crashReopenRecovered",
  "unexpectedTemporaryFilesAbsent",
];

export function backupReportPassesPolicy(report) {
  try {
    return backupReportPassesPolicyUnchecked(report);
  } catch {
    return false;
  }
}

function backupReportPassesPolicyUnchecked(report) {
  const topLevelKeys = [
    "schemaVersion",
    "scope",
    "status",
    "reason",
    "artifactExported",
    "recoveryOpenAvailable",
    "resourceWork",
    "productionComposition",
    "signedApplication",
    "nativeEvidenceClaimed",
    "closesGate",
    "signedNativeQualification",
  ];
  if (
    !hasExactDataKeys(report, topLevelKeys) ||
    report.schemaVersion !== 1 ||
    report.scope !== "p0-25-backup-foundation" ||
    report.status !== "unavailable" ||
    report.reason !==
      "sealed Host backup admission and reviewed safe SQLite handle/VFS identity are not composed" ||
    report.artifactExported !== false ||
    report.recoveryOpenAvailable !== false ||
    report.productionComposition !== false ||
    report.signedApplication !== false ||
    report.nativeEvidenceClaimed !== false ||
    report.closesGate !== false ||
    !hasExactDataKeys(report.resourceWork, ["filesystem", "database", "transaction", "crypto"]) ||
    Object.values(report.resourceWork).some((value) => value !== false) ||
    !hasExactDataKeys(report.signedNativeQualification, ["status", "platforms"]) ||
    report.signedNativeQualification.status !== "pending" ||
    !hasExactDataArray(report.signedNativeQualification.platforms, 3)
  ) {
    return false;
  }

  const expectedPlatforms = ["windows-11-24h2-x64", "macos-15-arm64", "macos-15-x64"];
  for (const [index, expectedPlatform] of expectedPlatforms.entries()) {
    const descriptor = Object.getOwnPropertyDescriptor(
      report.signedNativeQualification.platforms,
      String(index),
    );
    const platform = descriptor?.value;
    if (
      !hasExactDataKeys(platform, ["platform", "status", "signedArtifact", "report"]) ||
      platform.platform !== expectedPlatform ||
      platform.status !== "pending" ||
      platform.signedArtifact !== null ||
      platform.report !== null
    ) {
      return false;
    }
  }
  return true;
}

function hasExactDataKeys(value, expectedKeys) {
  if (
    value === null ||
    typeof value !== "object" ||
    Array.isArray(value) ||
    Object.getPrototypeOf(value) !== Object.prototype
  ) {
    return false;
  }
  const ownKeys = Reflect.ownKeys(value);
  if (
    ownKeys.length !== expectedKeys.length ||
    expectedKeys.some((key) => !ownKeys.includes(key))
  ) {
    return false;
  }
  return expectedKeys.every((key) => {
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
    return isEnumerableDataDescriptor(descriptor);
  });
}

function hasExactDataArray(value, expectedLength) {
  if (
    !Array.isArray(value) ||
    Object.getPrototypeOf(value) !== Array.prototype
  ) {
    return false;
  }
  const expectedKeys = [
    ...Array.from({ length: expectedLength }, (_, index) => String(index)),
    "length",
  ];
  const ownKeys = Reflect.ownKeys(value);
  if (
    ownKeys.length !== expectedKeys.length ||
    expectedKeys.some((key) => !ownKeys.includes(key))
  ) {
    return false;
  }
  return expectedKeys.every((key) => {
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
    return (
      isDataDescriptor(descriptor) &&
      (key === "length"
        ? descriptor.enumerable === false && descriptor.value === expectedLength
        : descriptor.enumerable === true)
    );
  });
}

function isDataDescriptor(descriptor) {
  return Boolean(
    descriptor &&
      Object.hasOwn(descriptor, "value") &&
      descriptor.get === undefined &&
      descriptor.set === undefined,
  );
}

function isEnumerableDataDescriptor(descriptor) {
  return isDataDescriptor(descriptor) && descriptor.enumerable === true;
}

function parseArguments(argv) {
  let profile = "local";
  let prepareOnly = false;
  let slice = null;

  for (let index = 0; index < argv.length; index += 1) {
    const argument = argv[index];
    if (argument === "--") {
      continue;
    }
    if (argument === "--prepare") {
      prepareOnly = true;
      continue;
    }
    if (argument === "--profile") {
      profile = argv[index + 1];
      index += 1;
      continue;
    }
    if (argument.startsWith("--profile=")) {
      profile = argument.slice("--profile=".length);
      continue;
    }
    if (argument === "--slice") {
      slice = argv[index + 1];
      index += 1;
      continue;
    }
    if (argument.startsWith("--slice=")) {
      slice = argument.slice("--slice=".length);
      continue;
    }
    throw new Error(`Unknown argument: ${argument}`);
  }

  if (!supportedProfiles.has(profile)) {
    throw new Error(
      `Unsupported profile "${profile}". Expected one of: ${[...supportedProfiles].join(", ")}.`,
    );
  }

  if (slice !== null && !supportedSlices.has(slice)) {
    throw new Error(
      `Unsupported slice "${slice}". Expected one of: ${[...supportedSlices].join(", ")}.`,
    );
  }

  return { prepareOnly, profile, slice };
}

function childEnvironment() {
  const environment = { ...process.env };
  delete environment.WP0_GITHUB_TOKEN;
  delete environment.EVIDENCE_GITHUB_TOKEN;
  return environment;
}

function run(binary, args, options = {}) {
  return spawnSync(binary, args, {
    cwd: root,
    encoding: "utf8",
    maxBuffer: 128 * 1024 * 1024,
    env: childEnvironment(),
    ...options,
  });
}

function probe(binary, args) {
  const result = run(binary, args, { timeout: 5_000 });
  if (result.status !== 0) {
    return "unavailable";
  }
  return result.stdout.trim();
}

function sha256(content) {
  return createHash("sha256").update(content).digest("hex");
}

function normalizedBackupInputPath(path) {
  if (typeof path !== "string" || path.length === 0 || isAbsolute(path) || /^[A-Za-z]:[\\/]/u.test(path)) {
    return null;
  }
  const normalized = path.replaceAll("\\", "/");
  if (normalized !== path || normalized.split("/").some((segment) => segment === "" || segment === "." || segment === "..")) {
    return null;
  }
  return normalized;
}

export function backupInputAdmissionErrors({
  repositoryRoot = root,
  inputPaths = BACKUP_EVIDENCE_INPUT_PATHS,
} = {}) {
  const errors = [];
  if (!Array.isArray(inputPaths) || inputPaths.length !== BACKUP_EVIDENCE_INPUT_PATHS.length) {
    return ["backup evidence input inventory must be the closed final-state list"];
  }
  for (const [index, path] of inputPaths.entries()) {
    const normalized = normalizedBackupInputPath(path);
    if (normalized === null) {
      errors.push(`backup evidence input is not a normalized repository-relative path: ${String(path)}`);
      continue;
    }
    if (normalized !== BACKUP_EVIDENCE_INPUT_PATHS[index]) {
      errors.push(`backup evidence input is not admitted: ${normalized}`);
      continue;
    }
    const absolutePath = resolve(repositoryRoot, normalized);
    try {
      const stat = lstatSync(absolutePath);
      if (stat.isSymbolicLink() || !stat.isFile()) {
        errors.push(`backup evidence input must be a regular non-symbolic-link file: ${normalized}`);
      }
    } catch (error) {
      errors.push(`backup evidence input is missing or unreadable: ${normalized}`);
    }
  }
  return errors;
}

function admittedBackupInputRecords() {
  const errors = backupInputAdmissionErrors();
  if (errors.length > 0) throw new Error(errors.join("\n"));
  return BACKUP_EVIDENCE_INPUT_PATHS.map((path) => {
    const content = readFileSync(resolve(root, path));
    return { path, bytes: content.length, sha256: sha256(content) };
  });
}

function splitNullTerminated(value) {
  return value.split("\0").filter(Boolean);
}

function commaSeparatedEnvironment(name) {
  return (process.env[name] ?? "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
}

function gitBytes(args) {
  const result = run("git", args, { encoding: null });
  return result.status === 0 ? result.stdout ?? Buffer.alloc(0) : null;
}

function hashUntrackedFiles(paths) {
  return paths.map((path) => {
    const absolutePath = resolve(root, path);
    const stat = lstatSync(absolutePath);
    if (stat.isSymbolicLink()) {
      const target = readlinkSync(absolutePath);
      return {
        path,
        kind: "symlink",
        mode: stat.mode,
        bytes: Buffer.byteLength(target),
        sha256: sha256(target),
      };
    }
    const content = readFileSync(absolutePath);
    return {
      path,
      kind: "file",
      mode: stat.mode,
      bytes: content.length,
      sha256: sha256(content),
    };
  });
}

function readRepositoryState() {
  const status = run("git", ["status", "--porcelain=v1", "--untracked-files=all"]);
  const pathCommands = [
    ["diff", "--name-only", "-z"],
    ["diff", "--cached", "--name-only", "-z"],
    ["ls-files", "--others", "--exclude-standard", "-z"],
  ];
  const changedPaths = new Set();
  let pathsAvailable = true;

  for (const args of pathCommands) {
    const result = run("git", args);
    if (result.status === 0) {
      for (const path of splitNullTerminated(result.stdout)) {
        changedPaths.add(path);
      }
    } else {
      pathsAvailable = false;
    }
  }

  const untracked = run("git", ["ls-files", "--others", "--exclude-standard", "-z"]);
  const untrackedPaths =
    untracked.status === 0 ? splitNullTerminated(untracked.stdout).sort() : null;
  const unstagedDiff = gitBytes(["diff", "--binary", "--no-ext-diff"]);
  const stagedDiff = gitBytes(["diff", "--cached", "--binary", "--no-ext-diff"]);
  const untrackedFiles = untrackedPaths === null ? [] : hashUntrackedFiles(untrackedPaths);
  const dirtyMaterial = {
    unstagedDiffSha256: unstagedDiff === null ? null : sha256(unstagedDiff),
    stagedDiffSha256: stagedDiff === null ? null : sha256(stagedDiff),
    untrackedFiles,
  };
  const statusAvailable = Boolean(
    status.status === 0 &&
      pathsAvailable &&
      untrackedPaths !== null &&
      unstagedDiff !== null &&
      stagedDiff !== null,
  );

  return {
    headCommit: probe("git", ["rev-parse", "HEAD"]),
    headTree: probe("git", ["rev-parse", "HEAD^{tree}"]),
    dirty:
      statusAvailable
        ? repositoryMaterialDirty({ stagedDiff, unstagedDiff, untrackedPaths })
        : null,
    statusAvailable,
    changedPaths: [...changedPaths].sort(),
    dirtyMaterial: {
      ...dirtyMaterial,
      contentDigest: sha256(JSON.stringify(dirtyMaterial)),
    },
  };
}

function repositoryInputStable(initial, final) {
  return Boolean(
    initial?.statusAvailable &&
      final?.statusAvailable &&
      initial.headCommit === final.headCommit &&
      initial.headTree === final.headTree &&
      initial.dirtyMaterial.contentDigest === final.dirtyMaterial.contentDigest,
  );
}

function technicalEligibility(manifest) {
  if (manifest.slice === "backup") {
    return {
      eligible: false,
      reasons: ["sealed-host-backup-unavailable"],
      scope:
        "repository-only unavailable backup evidence; never represents native, release, production, or Gate qualification",
    };
  }
  const reasons = [];
  const initial = manifest.repository.initial;
  const final = manifest.repository.final;
  if (manifest.phase !== "complete") {
    reasons.push("validation-not-complete");
  }
  if (
    !initial?.statusAvailable ||
    (final !== null && !final.statusAvailable) ||
    (manifest.phase === "complete" && final === null)
  ) {
    reasons.push("repository-state-unavailable");
  }
  if (initial?.dirty || final?.dirty) {
    reasons.push("dirty-worktree");
  }
  if (final && !repositoryInputStable(initial, final)) {
    reasons.push("repository-input-changed-during-run");
  }
  if (manifest.ci !== null && !manifest.ci.jobUrl) {
    reasons.push("ci-job-url-unavailable");
  }
  if (!manifest.executionContext.valid) {
    reasons.push("runner-context-mismatch");
  }
  if (manifest.validations.some((validation) => validation.exitCode !== 0)) {
    reasons.push("validation-failed");
  }
  if (!sliceEvidenceValid(manifest.sliceEvidence)) {
    reasons.push("slice-evidence-invalid");
  }
  return {
    eligible: reasons.length === 0,
    reasons,
    scope: "technical-evidence-only; never represents Gate closure",
  };
}

function sliceEvidenceValid(sliceEvidence) {
  if (sliceEvidence === null) {
    return true;
  }
  const report = sliceEvidence.report;
  if (sliceEvidence.slice === "account-gate") {
    return Boolean(
      sliceEvidence.present &&
        report?.schemaVersion === 1 &&
        report.fixtureOnly === true &&
        report.productionRoutesRegistered === false &&
        report.rawCredentialFieldsAbsent === true &&
        report.internalAccountSellable === false &&
        report.requiredInputsPresent === true &&
        report.accountVectors?.valid > 0 &&
        report.accountVectors?.invalid > 0 &&
        report.deviceVectors?.valid > 0 &&
        report.deviceVectors?.invalid > 0 &&
        Array.isArray(report.inputs) &&
        report.inputs.every(
          (input) => input.bytes > 0 && /^[0-9a-f]{64}$/.test(input.sha256),
        )
    );
  }
  if (sliceEvidence.slice === "cloud-boundary") {
    return Boolean(sliceEvidence.present && cloudBoundaryReportPassesPolicy(report));
  }
  if (sliceEvidence.slice === "backup") {
    return Boolean(sliceEvidence.present && backupReportPassesPolicy(report));
  }
  if (sliceEvidence.slice === "jobs") {
    return Boolean(
      sliceEvidence.present &&
        report?.schemaVersion === 1 &&
        report.periodicJobsRegistered === false &&
        report.crossTenant?.passed === true &&
        report.replay?.passed === true &&
        report.idempotency?.passed === true &&
        report.leaseContention?.passed === true &&
        report.crossTenant?.count > 0 &&
        report.replay?.count > 0 &&
        report.idempotency?.count > 0 &&
        report.leaseContention?.count > 0,
    );
  }
  return Boolean(
    sliceEvidence.present &&
      report?.schemaVersion === 1 &&
      report.sqlcipherVersion === "4.17.0 community" &&
      report.sqliteVersion === "3.53.3" &&
      sqlcipherRequiredChecks.every((check) => report[check] === true) &&
      Array.isArray(report.scans) &&
      report.scans.length >= 7 &&
      report.scans.every(
        (scan) =>
          scan.bytes > 0 &&
          /^[0-9a-f]{64}$/.test(scan.sha256) &&
          scan.canaryAbsent === true &&
          scan.sqliteHeaderAbsent === true,
      ),
  );
}

function evidenceEnvironment(name) {
  return process.env[`EVIDENCE_${name}`] ?? process.env[`WP0_${name}`];
}

function readExecutionContext() {
  const expectedPlatform = evidenceEnvironment("EXPECTED_PLATFORM") ?? null;
  const expectedArch = evidenceEnvironment("EXPECTED_ARCH") ?? null;
  const actualPlatform = platform();
  const actualArch = arch();
  const ci = process.env.GITHUB_ACTIONS === "true";
  const expectedContextPresent = Boolean(expectedPlatform && expectedArch);

  return {
    expected: { platform: expectedPlatform, arch: expectedArch },
    actual: {
      platform: actualPlatform,
      arch: actualArch,
      runnerOs: process.env.RUNNER_OS ?? null,
      runnerArch: process.env.RUNNER_ARCH ?? null,
      imageOs: process.env.ImageOS ?? null,
      imageVersion: process.env.ImageVersion ?? null,
    },
    valid:
      !ci ||
      (expectedContextPresent &&
        expectedPlatform === actualPlatform &&
        expectedArch === actualArch),
  };
}

function hashKeyInputs(slice) {
  if (slice === "backup") return admittedBackupInputRecords();
  const paths = [
    ...keyInputPaths,
    ...(slice === "account-gate" ? accountGateKeyInputPaths : []),
    ...(slice === "cloud-boundary" ? cloudBoundaryKeyInputPaths : []),
  ];
  return paths.map((path) => {
    const absolutePath = resolve(root, path);
    if (!existsSync(absolutePath)) {
      return { path, present: false, sha256: null };
    }
    const content = readFileSync(absolutePath);
    return { path, present: true, sha256: sha256(content) };
  });
}

function readSliceEvidence(slice, reportPath) {
  if (slice === null) {
    return null;
  }
  if (!existsSync(reportPath)) {
    return {
      slice,
      reportPath: relative(root, reportPath),
      present: false,
      sha256: null,
      report: null,
    };
  }
  const content = readFileSync(reportPath);
  let report;
  let parseError = null;
  try {
    report = JSON.parse(content.toString("utf8"));
  } catch (error) {
    report = null;
    parseError = error instanceof Error ? error.message : String(error);
  }
  return {
    slice,
    reportPath: relative(root, reportPath),
    present: true,
    sha256: sha256(content),
    parseError,
    report,
  };
}

function profileDefinition(profile, slice, reportPath) {
  const pnpm = (args) => platformCommand("pnpm", args);
  const common = [
    {
      id: "platform-neutral-checks",
      ...pnpm(["check:platform-neutral"]),
    },
    { id: "rust-format", binary: "cargo", args: ["fmt", "--all", "--check"] },
  ];
  const portableRust = [
    {
      id: "portable-rust-lint",
      binary: "cargo",
      args: ["clippy", ...portableCrates, "--", "-D", "warnings"],
    },
    {
      id: "portable-rust-tests",
      binary: "cargo",
      args: ["test", ...portableCrates],
    },
  ];
  const nativeRust = [
    {
      id: "native-rust-lint",
      binary: "cargo",
      args: ["clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    },
    {
      id: "native-rust-tests",
      binary: "cargo",
      args: ["test", "--workspace", "--all-targets"],
    },
  ];

  if (slice === "cloud-boundary") {
    return {
      resolvedProfile: "wp4-cloud-boundary",
      applicability:
        "Fixture-only Cloud entrypoint isolation, error identity, OpenAPI, boundary policy, and single-process rate-limit evidence; no business route, real credential, persistence, external network, AuditEvent chain, or job runtime.",
      commands: [
        {
          id: "protocol-typecheck",
          ...pnpm(["--filter", "@gooddealer/protocol", "typecheck"]),
        },
        {
          id: "protocol-contract-tests",
          ...pnpm(["--filter", "@gooddealer/protocol", "test"]),
        },
        {
          id: "cloud-entrypoint-typecheck",
          ...pnpm(["--filter", "@gooddealer/cloud", "typecheck"]),
        },
        {
          id: "cloud-entrypoint-tests",
          ...pnpm(["--filter", "@gooddealer/cloud", "test"]),
        },
        {
          id: "cloud-boundary-policy-tests",
          binary: "node",
          args: [
            "--test",
            "scripts/cloud-boundary-evidence-policy.test.mjs",
            "scripts/boundary-policy.test.mjs",
          ],
        },
        {
          id: "dependency-boundary-check",
          binary: "node",
          args: ["scripts/check-boundaries.mjs"],
        },
        {
          id: "cloud-boundary-report",
          binary: "node",
          args: ["scripts/collect-cloud-boundary-report.mjs", reportPath],
        },
      ],
    };
  }

  if (slice === "jobs") {
    return {
      resolvedProfile: "wp4-jobs",
      applicability:
        "Fixture-only TenantJobEnvelope, Lease, Idempotency and Quarantine evidence; no periodic Job, real credential, scheduler, network, or production Route.",
      commands: [
        {
          id: "protocol-typecheck",
          ...pnpm(["--filter", "@gooddealer/protocol", "typecheck"]),
        },
        {
          id: "protocol-contract-tests",
          ...pnpm(["--filter", "@gooddealer/protocol", "test"]),
        },
        {
          id: "cloud-fixture-typecheck",
          ...pnpm(["--filter", "@gooddealer/cloud", "typecheck"]),
        },
        {
          id: "cloud-fixture-tests",
          ...pnpm(["--filter", "@gooddealer/cloud", "test"]),
        },
        {
          id: "jobs-evidence-policy-tests",
          binary: "node",
          args: ["--test", "scripts/jobs-evidence-policy.test.mjs"],
        },
        {
          id: "jobs-report",
          binary: "node",
          args: ["scripts/collect-jobs-report.mjs", reportPath],
        },
      ],
    };
  }

  if (slice === "account-gate") {
    return {
      resolvedProfile: "wp2-account-gate-cloud-fixture",
      applicability:
        "Portable account/device/bootstrap/workspace contract and Cloud fixture evidence; no production routes, lease signing, raw credentials, sellable accounts, native secret storage, or external network.",
      commands: [
        {
          id: "protocol-typecheck",
          ...pnpm(["--filter", "@gooddealer/protocol", "typecheck"]),
        },
        {
          id: "protocol-contract-tests",
          ...pnpm(["--filter", "@gooddealer/protocol", "test"]),
        },
        {
          id: "cloud-fixture-typecheck",
          ...pnpm(["--filter", "@gooddealer/cloud", "typecheck"]),
        },
        {
          id: "cloud-fixture-tests",
          ...pnpm(["--filter", "@gooddealer/cloud", "test"]),
        },
        {
          id: "client-core-typecheck",
          ...pnpm(["--filter", "@gooddealer/client-core", "typecheck"]),
        },
        {
          id: "client-core-tests",
          ...pnpm(["--filter", "@gooddealer/client-core", "test"]),
        },
        {
          id: "dependency-boundary-check",
          binary: "node",
          args: ["scripts/check-boundaries.mjs"],
        },
        {
          id: "account-gate-fixture-report",
          binary: "node",
          args: ["scripts/collect-account-gate-report.mjs", reportPath],
        },
      ],
    };
  }

  if (slice === "backup") {
    return {
      resolvedProfile: "wp5-backup-repository-unavailable",
      applicability:
        "Repository-only unavailable backup evidence: sealed Host admission and reviewed SQLite handle/VFS identity are not composed, so no backup artifact, recovery open, resource work, production composition, native qualification, release evidence, or Gate closure is asserted.",
      commands: [
        { id: "rust-format", binary: "cargo", args: ["fmt", "--all", "--check"] },
        {
          id: "backup-unavailable-lint",
          binary: "cargo",
          args: [
            "clippy",
            "--locked",
            "-p",
            "gooddealer-local-storage",
            "--features",
            "backup-foundation-evidence",
            "--all-targets",
            "--",
            "-D",
            "warnings",
          ],
        },
        {
          id: "backup-unavailable-tests",
          binary: "cargo",
          args: [
            "test",
            "--locked",
            "-p",
            "gooddealer-local-storage",
            "--features",
            "backup-foundation-evidence",
            "--all-targets",
          ],
        },
        {
          id: "backup-unavailable-policy",
          binary: "node",
          args: ["--test", "scripts/backup-evidence-policy.test.mjs"],
        },
        {
          id: "backup-unavailable-report",
          binary: "cargo",
          args: [
            "run",
            "--locked",
            "-p",
            "gooddealer-local-storage",
            "--example",
            "backup_evidence",
            "--features",
            "backup-foundation-evidence",
            "--",
            reportPath,
          ],
        },
      ],
    };
  }

  if (slice === "sqlcipher") {
    if (profile === "quality") {
      throw new Error("The sqlcipher slice requires a native or local profile.");
    }
    return {
      resolvedProfile: profile === "native" ? "wp5-sqlcipher-native" : "wp5-sqlcipher-local",
      applicability:
        "Test-only SQLCipher fixture on a supported native host; no production database, credentials, bundle, signing, or user data.",
      commands: [
        { id: "rust-format", binary: "cargo", args: ["fmt", "--all", "--check"] },
        {
          id: "sqlcipher-fixture-lint",
          binary: "cargo",
          args: [
            "clippy",
            "--locked",
            "-p",
            "gooddealer-local-storage",
            "--all-targets",
            "--",
            "-D",
            "warnings",
          ],
        },
        {
          id: "sqlcipher-fixture-tests",
          binary: "cargo",
          args: ["test", "--locked", "-p", "gooddealer-local-storage", "--all-targets"],
        },
        {
          id: "sqlcipher-fault-and-plaintext-evidence",
          binary: "cargo",
          args: [
            "run",
            "--locked",
            "--release",
            "-p",
            "gooddealer-local-storage",
            "--example",
            "sqlcipher_evidence",
            "--",
            reportPath,
          ],
        },
      ],
    };
  }

  if (profile === "quality") {
    return {
      resolvedProfile: "quality",
      applicability: "Platform-neutral checks for the Ubuntu quality runner.",
      commands: [
        {
          id: "dependency-vulnerability-gate",
          ...pnpm(["audit", "--audit-level", "high"]),
        },
        {
          id: "rust-dependency-vulnerability-gate",
          binary: "cargo",
          args: ["audit"],
        },
        ...common,
        ...portableRust,
      ],
    };
  }

  if (profile === "native") {
    return {
      resolvedProfile: "native",
      applicability:
        "Native compile checks on a supported desktop runner; this does not build, bundle, sign, or publish a Tauri application artifact.",
      commands: [
        ...common,
        {
          id: "desktop-frontend-build",
          ...pnpm(["--filter", "@gooddealer/desktop", "build"]),
        },
        ...nativeRust,
      ],
    };
  }

  if (platform() === "linux") {
    return {
      resolvedProfile: "local-portable",
      applicability:
        "Linux is not a supported desktop target; local evidence excludes desktop and non-portable workspace crates.",
      commands: [...common, ...portableRust],
    };
  }

  return {
    resolvedProfile: "local-native",
    applicability:
      "Local native compile checks for a supported macOS or Windows host; this is not a Tauri release or bundle build.",
    commands: [
      ...common,
      {
        id: "desktop-frontend-build",
        ...pnpm(["--filter", "@gooddealer/desktop", "build"]),
      },
      ...nativeRust,
    ],
  };
}

async function resolveCiEvidence() {
  const serverUrl = process.env.GITHUB_SERVER_URL;
  const repository = process.env.GITHUB_REPOSITORY;
  const runId = process.env.GITHUB_RUN_ID;
  if (!serverUrl || !repository || !runId) {
    return null;
  }

  const runAttempt = process.env.GITHUB_RUN_ATTEMPT ?? "1";
  const jobName = evidenceEnvironment("CI_JOB_NAME") ?? process.env.GITHUB_JOB ?? null;
  const runUrl = `${serverUrl}/${repository}/actions/runs/${runId}`;
  const ci = {
    provider: "github-actions",
    workflow: process.env.GITHUB_WORKFLOW ?? null,
    runId,
    runAttempt,
    runUrl,
    jobKey: process.env.GITHUB_JOB ?? null,
    jobName,
    jobUrl: null,
    jobUrlResolution: "unavailable",
  };
  const token = evidenceEnvironment("GITHUB_TOKEN");
  const apiUrl = process.env.GITHUB_API_URL;
  if (!token || !apiUrl || !jobName) {
    ci.jobUrlResolution = "missing-api-context";
    return ci;
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 5_000);
  try {
    const response = await fetch(
      `${apiUrl}/repos/${repository}/actions/runs/${runId}/attempts/${runAttempt}/jobs?per_page=100`,
      {
        headers: {
          Accept: "application/vnd.github+json",
          Authorization: `Bearer ${token}`,
          "X-GitHub-Api-Version": "2022-11-28",
        },
        signal: controller.signal,
      },
    );
    if (!response.ok) {
      ci.jobUrlResolution = `github-api-http-${response.status}`;
      return ci;
    }
    const payload = await response.json();
    const jobs = Array.isArray(payload.jobs) ? payload.jobs : [];
    const exactMatch = jobs.find((job) => job.name === jobName);
    const keyMatch = jobs.find((job) => job.name === ci.jobKey);
    const currentJob = exactMatch ?? keyMatch;
    if (currentJob?.html_url) {
      ci.jobUrl = currentJob.html_url;
      ci.jobUrlResolution = "github-api";
    } else {
      ci.jobUrlResolution = "job-not-found";
    }
  } catch (error) {
    ci.jobUrlResolution = error instanceof Error ? error.name : "github-api-error";
  } finally {
    clearTimeout(timeout);
  }

  return ci;
}

function executeValidation(definition, index, logDirectory) {
  const command = definition;
  const sequence = String(index + 1).padStart(2, "0");
  const logStem = `${sequence}-${command.id}`;
  const stdoutPath = resolve(logDirectory, `${logStem}.stdout.log`);
  const stderrPath = resolve(logDirectory, `${logStem}.stderr.log`);
  const startedAt = new Date();
  const result = spawnSync(command.binary, command.args, {
    cwd: root,
    encoding: null,
    maxBuffer: 128 * 1024 * 1024,
    env: childEnvironment(),
  });
  const finishedAt = new Date();
  const stdout = result.stdout ?? Buffer.alloc(0);
  let stderr = result.stderr ?? Buffer.alloc(0);
  if (result.error) {
    stderr = Buffer.concat([stderr, Buffer.from(`${result.error.stack ?? result.error.message}\n`)]);
  }
  const exitCode = result.status ?? (result.error?.code === "ENOENT" ? 127 : 1);

  writeFileSync(stdoutPath, stdout);
  writeFileSync(stderrPath, stderr);
  if (stdout.length > 0) {
    process.stdout.write(stdout);
  }
  if (stderr.length > 0) {
    process.stderr.write(stderr);
  }

  return {
    id: command.id,
    command: [command.binary, ...command.args],
    startedAt: startedAt.toISOString(),
    finishedAt: finishedAt.toISOString(),
    durationMs: finishedAt.getTime() - startedAt.getTime(),
    exitCode,
    signal: result.signal,
    stdout: {
      path: relative(root, stdoutPath),
      bytes: stdout.length,
      sha256: sha256(stdout),
    },
    stderr: {
      path: relative(root, stderrPath),
      bytes: stderr.length,
      sha256: sha256(stderr),
    },
  };
}

if (isDirectInvocation()) {
let options;
try {
  options = parseArguments(process.argv.slice(2));
} catch (error) {
  console.error(error instanceof Error ? error.message : error);
  process.exitCode = 2;
  options = { prepareOnly: true, profile: "local", slice: null };
}

if (options.slice === "backup") {
  const inputErrors = backupInputAdmissionErrors();
  if (inputErrors.length > 0) {
    throw new Error(inputErrors.join("\n"));
  }
}

const workPackage =
  options.slice === "account-gate"
    ? "wp2"
    : options.slice === "cloud-boundary" || options.slice === "jobs"
      ? "wp4"
      : options.slice === "backup" || options.slice === "sqlcipher"
        ? "wp5"
        : "wp0";
const outputDirectory = resolve(
  root,
  options.slice === null
    ? `.artifacts/${workPackage}`
    : `.artifacts/${workPackage}/${options.slice}`,
);
const logDirectory = resolve(outputDirectory, "logs");
const manifestPath = resolve(outputDirectory, "evidence.json");
const sliceReportPath = resolve(
  outputDirectory,
  options.slice === "account-gate"
    ? "account-gate-report.json"
    : options.slice === "backup"
      ? "backup-report.json"
    : options.slice === "cloud-boundary"
      ? "cloud-boundary-report.json"
    : options.slice === "jobs"
      ? "jobs-report.json"
    : "sqlcipher-report.json",
);
const definition = profileDefinition(options.profile, options.slice, sliceReportPath);
const pnpmProbe = platformCommand("pnpm", ["--version"]);
const startedAt = new Date();
mkdirSync(logDirectory, { recursive: true });

const manifest = {
  schemaVersion: 1,
  workPackage: workPackage.toUpperCase().replace("WP", "WP-"),
  slice: options.slice,
  phase: options.prepareOnly ? "prepared" : "running",
  profile: {
    requested: options.profile,
    resolved: definition.resolvedProfile,
    applicability: definition.applicability,
  },
  artifactClaims: {
    tauriReleaseBuild: false,
    bundle: false,
    signedApplication: false,
    applicationArtifact: false,
    releaseModeFixtureValidation: options.slice === "sqlcipher",
    temporarySqlcipherFixture: options.slice === "sqlcipher",
    ...(options.slice === "account-gate"
      ? {
          accountGateFixture: true,
          productionAccountRoute: false,
          rawCredentials: false,
        }
      : {}),
    ...(options.slice === "cloud-boundary"
      ? {
          cloudBoundaryFixture: true,
          publicBusinessRoute: false,
          adminBusinessRoute: false,
          periodicJob: false,
          auditEventChain: false,
          jobRuntime: false,
        }
      : {}),
    ...(options.slice === "jobs"
      ? {
          jobRuntimeFixture: true,
          periodicJob: false,
          schedulerWiring: false,
          productionRoute: false,
        }
      : {}),
    productionStorage: false,
    userData: false,
  },
  commit: probe("git", ["rev-parse", "HEAD"]),
  generatedAt: null,
  startedAt: startedAt.toISOString(),
  finishedAt: null,
  os: { platform: platform(), release: release(), arch: arch() },
  executionContext: readExecutionContext(),
  tools: {
    node: process.version,
    pnpm: probe(pnpmProbe.binary, pnpmProbe.args),
    rustc: probe("rustc", ["--version"]),
    cargo: probe("cargo", ["--version"]),
    cargoAudit: probe("cargo", ["audit", "--version"]),
  },
  ci: await resolveCiEvidence(),
  responsibility: {
    ownerRole:
      evidenceEnvironment("OWNER_ROLE") ??
      (options.slice === "account-gate"
        ? "Account Access and Cloud Devices Lead"
        : options.slice === "cloud-boundary" || options.slice === "jobs"
          ? CLOUD_BOUNDARY_EVIDENCE_DEFAULTS.ownerRole
        : "Release Engineering Lead"),
    owningModules: commaSeparatedEnvironment("EVIDENCE_OWNING_MODULES").length
      ? commaSeparatedEnvironment("EVIDENCE_OWNING_MODULES")
      : commaSeparatedEnvironment("WP0_OWNING_MODULES").length
        ? commaSeparatedEnvironment("WP0_OWNING_MODULES")
        : options.slice === "account-gate"
          ? ["protocol-account", "protocol-devices", "protocol-workspace", "cloud-account-device-bootstrap-fixtures", "client-core"]
          : options.slice === "cloud-boundary"
            ? [...CLOUD_BOUNDARY_EVIDENCE_DEFAULTS.owningModules]
          : options.slice === "jobs"
            ? ["protocol-jobs", "cloud-job-runtime"]
          : options.slice === "sqlcipher"
          ? ["local-storage", "recovery", "release-engineering"]
          : ["engineering-baseline", "release-engineering"],
    requiredReviewerRole:
      evidenceEnvironment("REQUIRED_REVIEWER_ROLE") ??
      (options.slice === "account-gate" || options.slice === "cloud-boundary" || options.slice === "jobs"
        ? CLOUD_BOUNDARY_EVIDENCE_DEFAULTS.requiredReviewerRole
        : "Architecture Reviewer"),
    approverRole: evidenceEnvironment("APPROVER_ROLE") ?? "Phase 0 Gate Approver",
    evidenceProducerRef: process.env.GITHUB_ACTOR_ID
      ? `github-user-id:${process.env.GITHUB_ACTOR_ID}`
      : process.env.GITHUB_ACTOR
        ? `github-login:${process.env.GITHUB_ACTOR}`
        : null,
  },
  closureAttestationRequirements: {
    required: true,
    schemaVersion: 1,
    manifestDigestAlgorithm: "sha256",
    requiredFields: [
      "schema_version",
      "attestation_id",
      "gate_id",
      "subject_commit",
      "implementation_refs",
      "evidence_sets",
      "owner",
      "reviews",
      "approval",
    ],
    evidenceSetRequiredFields: [
      "profile",
      "platform",
      "architecture",
      "job_name",
      "manifest_sha256",
      "artifact",
      "archive",
    ],
    evidenceArtifactRequiredFields: [
      "artifact_id",
      "artifact_digest",
      "run_url",
      "job_url",
    ],
    evidenceArchiveRequiredFields: [
      "archive_ref",
      "archive_sha256",
      "verified_at",
      "verified_by_ref",
      "retention_policy",
    ],
    ownerRequiredFields: ["role", "ref"],
    reviewRequiredFields: [
      "role",
      "ref",
      "reviewed_at",
      "independence_assertion",
    ],
    approvalRequiredFields: ["role", "ref", "approved_at"],
    gateSpecificEvidenceSetCoverageRequired: true,
    gateSpecificRoleCoverageRequired: true,
    independenceRules: [
      "implementer-must-not-be-sole-reviewer",
      "implementer-must-not-self-approve",
      "security-gates-require-independent-security-reviewer",
    ],
  },
  repository: {
    initial: readRepositoryState(),
    final: null,
  },
  keyInputs: hashKeyInputs(options.slice),
  sliceEvidence: readSliceEvidence(options.slice, sliceReportPath),
  validations: [],
  result: options.prepareOnly ? "not-run" : "pending",
  technicalEligibility: {
    eligible: false,
    reasons: ["validation-not-complete"],
    scope: "technical-evidence-only; never represents Gate closure",
  },
};

manifest.technicalEligibility = technicalEligibility(manifest);

function writeManifest() {
  manifest.generatedAt = new Date().toISOString();
  writeFileSync(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);
}

// Write an initial artifact before any validation command so an interrupted run
// still leaves an explicit incomplete evidence record.
writeManifest();

if (!options.prepareOnly && process.exitCode !== 2) {
  for (const [index, command] of definition.commands.entries()) {
    try {
      manifest.validations.push(executeValidation(command, index, logDirectory));
    } catch (error) {
      manifest.validations.push({
        id: command.id,
        command: [command.binary, ...command.args],
        exitCode: 1,
        collectionError: error instanceof Error ? error.message : String(error),
      });
    }
    manifest.sliceEvidence = readSliceEvidence(options.slice, sliceReportPath);
    writeManifest();
  }
  manifest.repository.final = readRepositoryState();
  manifest.finishedAt = new Date().toISOString();
  manifest.phase = "complete";
  manifest.technicalEligibility = technicalEligibility(manifest);
  const validationsPassed = manifest.validations.every(
    (validation) => validation.exitCode === 0,
  );
  const sliceEvidencePassed = sliceEvidenceValid(manifest.sliceEvidence);
  const ciRepositoryValid =
    manifest.ci === null ||
    (!manifest.repository.initial.dirty &&
      !manifest.repository.final.dirty &&
      repositoryInputStable(manifest.repository.initial, manifest.repository.final) &&
      Boolean(manifest.ci.jobUrl));
  const runPassed =
    validationsPassed &&
    sliceEvidencePassed &&
    ciRepositoryValid &&
    manifest.executionContext.valid;
  manifest.result =
    options.slice === "backup"
      ? runPassed
        ? "unavailable"
        : "failed"
      : runPassed
        ? "passed"
        : "failed";
  writeManifest();
  process.exitCode = manifest.result === "failed" ? 1 : 0;
}

console.log(JSON.stringify(manifest, null, 2));
}

function isDirectInvocation() {
  const entrypoint = process.argv[1];
  return entrypoint !== undefined && resolve(entrypoint) === fileURLToPath(import.meta.url);
}
