...
export function renderTs(registry, hash) {
  ...
  const publicResponses = registry.filter((endpoint) => endpoint.responseExtractor === "public_json");
  const responses = publicResponses.length === 0
    ? "never"
    : publicResponses.map((endpoint) => `{ endpointId: ${JSON.stringify(endpoint.endpointId)}; body: ${tsObjectType(endpoint.publicResponseSchema.fields)} }`).join("\n  | ");
