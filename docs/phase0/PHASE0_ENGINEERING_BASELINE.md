...
R0-11 使用 [严格模板](attestations/R0_11_GATE_CLOSURE_ATTESTATION.template.json) 和 `pnpm gate:validate -- <attestation.json>`。Validator 对 schema、五份独立 evidence set（含最终 Windows 11 24H2 真机集）、稳定身份、实现者/Reviewer/Approver 独立性、归档唯一性、非 Actions 临时 URL、归档复核时间与未过期保留策略失败关闭。模板中的占位值本身不能通过；在真实身份和长期不可变归档尚未提供时，R0-11 必须保持 `In Progress`。
...
`pnpm evidence:wp2` 复用同一 provenance、仓库稳定性和精确 CI Job URL 语义，写入 `.artifacts/wp2/account-gate`。它验证 account/device/workspace 协议正负 Corpus、Cloud `identity/licensing/devices/workspace` 内部不可售 Fixture、client-core RuntimeMode 只读投影和依赖边界，并记录关键输入 SHA-256；报告失败关闭地检查责任模块未注册生产 Route、未签发 Lease、Fixture 账号不可售，以及原始密码不进入 shared protocol、持久化状态、响应、日志、错误或其他禁止面。Cloud `identity` 内部密码命令只能调用不能表达成功的 `DenyingPasswordHashPort`；该切片不接真实密码验证、品牌化 WebView→write-only IPC、Keychain、生产 Route、外部网络、真实凭据或用户数据，也不证明持久化 Cloud 事务、Windows/macOS 原生行为或 R0-06/R0-16 Gate 关闭。

P0-23 的 Portable Query Contract 同样进入 `pnpm evidence:wp2`：报告把 `protocol/workspace` 投影 Codec、client-core Query Port/Fixture Adapter、Contract Suite 与独立摘要 Corpus 纳入关键输入，并失败关闭确认 Port 只有 `listDomains`、Active/Standby 来源与 `canEdit` 不可互换、共享业务投影摘要逐字节一致，以及生产 Tauri/local-storage、cloud-client/workspace-read 接线仍不存在。它不是生产 Query Boundary 或 Composition Root 证据。
