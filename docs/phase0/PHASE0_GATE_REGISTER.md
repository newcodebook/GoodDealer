...
R0-11 由 `pnpm evidence:wp0`（本地默认 Profile）或 `node scripts/collect-wp0-evidence.mjs --profile quality|native` 生成 Evidence Manifest。Manifest 记录责任角色/模块、Evidence Producer 稳定身份（环境可提供时）、当前 commit/tree、dirty path、staged/unstaged diff Hash、逐个未跟踪文件内容 Hash、预期/实际 OS 与架构、Runner Image、工具版本、命令退出码、输出 Hash、关键输入 Hash 和 CI Job URL。CI 中无法解析精确 Job URL或 Runner 上下文不匹配时必须分别产生 `ci-job-url-unavailable` 或 `runner-context-mismatch`，技术资格和结果均失败；`runUrl + jobName` 不能静默替代精确 Job provenance。初始或最终 dirty 的本地运行可以作为诊断记录，但必须标记 `technicalEligibility.eligible=false`；CI 技术证据还强制初始/最终工作树干净且 commit/tree/输入内容摘要完全一致，否则即使命令退出码全为零也判定失败。`technicalEligibility` 永远不表示 Gate 关闭。

关闭时为 R0-11 单独建立 schema v1 GateClosureAttestation，绑定 `gate_id/subject_commit/implementation_refs/evidence_sets[{profile,platform,architecture,job_name,manifest_sha256,artifact{artifact_id,artifact_digest,run_url,job_url},archive{archive_ref,archive_sha256,verified_at,verified_by_ref,retention_policy}}]/owner{role,ref}/reviews[{role,ref,reviewed_at,independence_assertion}]/approval{role,ref,approved_at}`。Gate-specific Validator 已由 `pnpm gate:validate -- <attestation.json>` 与 [严格模板](attestations/R0_11_GATE_CLOSURE_ATTESTATION.template.json) 建立，按 §1/§2.1 校验完整 Job 集合、全部角色、实现者独立性和不可变归档元数据；任何占位值、Actions 临时 URL、缺失或复用的 evidence set、实现者自审/自批、过期保留策略或未覆盖全部实现者的独立性声明都会失败关闭。

R0-11 的集合固定包含 `quality`、native 的 `windows-server-2025-x64-compile`、`macos-15-arm64`、`macos-15-intel`，以及独立的 `windows-11-24h2-real-install` 五个 evidence set；Windows Server compile 不能替代 Windows 11 24H2 真机打包/WebView2。Workflow Artifact 名包含 Run ID/Attempt，90 天 Artifact 只作为评审传输与短期复跑材料，关闭前必须把每份逐字节相同证据包提升到覆盖 Gate 与项目审计期的不可变归档。在完整集合、长期归档、独立 Attestation 与制品可追溯前，R0-11 保持 `In Progress`。
...
P0-15/P0-19 的 `account-gate` 纵切已建立 `pnpm evidence:wp2`：Ubuntu Hosted Workflow 运行 account/device Wire 正负 Corpus、Cloud `identity/licensing/devices` 内部不可售 Fixture、client-core RuntimeMode 只读投影与依赖边界检查，并生成输入 Hash 和失败关闭的 Fixture 范围报告。Cloud `identity` 内部已有接收账号密码的窄命令边界，但默认且唯一允许的 `DenyingPasswordHashPort` 不能表达验证成功；证据逐面确认原始密码不进入 shared protocol、持久化状态、响应、日志、错误或其他禁止面。Auth Session Envelope、Refresh Family/JTI 轮换与 Secure Host 内存/凭据存储 Port 已形成 Portable Fixture；真实密码验证、品牌化 WebView→write-only IPC、OS Keychain、生产 Route/网络、持久化 Cloud 事务与 Windows/macOS 原生行为仍未交付。在这些缺口、独立 Security Review 与 GateClosureAttestation 完成前，R0-06/R0-16 保持 In Progress，账号入口仍为不可售 Fixture。

P0-16 的编排设计已经冻结，提交 `cc5401e`、`7923ca1`、`0a506a6`、`2c57116`、`77a87fc`、`52a70a8`、`26bc6f4` 已形成 C0～C2/D2 技术集合：账号/Lease 与 bootstrap-step DTO 具备 TS/Rust fail-closed 镜像，Cloud Fixture 覆盖账号级 DeviceSwitch/Bootstrap/Lease 互斥、strict-step nonce/CAS/逐字节幂等、期限清理、Cursor 与烧毁 Epoch 守护，client-core 覆盖纯投影与重建消费，`evidence:wp2` 覆盖无 Lease 签发等负向边界。该集合未注册生产 Route，不验证真实 Capability 签名，不提供持久化 Cloud 事务或可成功签发 ActiveDeviceLease 的路径；P0-16/R0-05/R0-06/R0-16 均保持 In Progress，Fallback 仍是不签发 Lease。

P0-17 的三流 Drain 设计已经冻结，S1～S7 已形成当前 Portable 技术集合：协议 Golden Corpus 固定三个独立序列域、canonical envelope、`sha256-chain-v1`、DrainProof/DrainManifest purpose 隔离、服务端富化摘要不变性，以及最大深度 32、最多 4096 个值节点的递归 redacted payload 边界；Cloud Fixture 覆盖十三步验证、逐流 Gap/水位/摘要、Late/quarantine/drain-seal，并将 Proof 消费、三流 seal、Lease 释放、Epoch 分配和 workflow transition 纳入同一补偿事务；client-core 覆盖纯 Drain ledger、claims builder 和 readiness。S6 已纳入 `evidence:wp2` 的结构化 Fallback 证据；S7 Rust Drain Envelope 镜像及共享 TypeScript/Rust Corpus 由 Rust 测试和根 `pnpm check` 覆盖，但 dedicated `evidence:wp2` 尚未执行 Rust。真实签名验证、生产持久化与 native/远端对应证据仍未进入该技术集合，签名端口仍不能表达成功；P0-17/R0-05/R0-16 均保持 In Progress，正常 handoff 继续禁止。

P0-20 已形成当前内存型 Cloud Fixture 纵切：按 `account_id + workspace_id` 强制租户作用域的 Mutation Ingest 在全部校验后以 CAS 分配连续 `serverRevision`，失败和重放不烧 Revision；Checkpoint 已覆盖 build→verify→publish、Bootstrap pin、保留链与前缀压缩，ReaderCursor 已覆盖 TTL、续租、设备移除、压缩竞态和 `rebootstrap_required | none` 语义，双租户同字面 ID 与 Standby 无写能力均有负向矩阵。该集合仍使用内存 Fixture，不包含持久化 Repository/native DB、Candidate/RestoreCandidate CAS、完整业务实体或生产 Route；P0-20/R0-04/R0-05 保持 In Progress，Fallback 仍是不可上传的本地/Cloud Fixture。

P0-23 已形成 Portable Query Contract 纵切：`client-core/portfolio` 的唯一只读 Port 与 Active Local/Standby Cloud Fixture Adapter 从 `unknown` 解析同一非秘密 `domain_asset` 投影，DataFreshness 固定来源、Server Revision、最后云同步/平台读取时间及 `canEdit`，并复用 `protocol/workspace` UTF-8 排序和摘要 Corpus 验证业务投影等价。未知/秘密字段、unsafe Revision、非规范排序、来源冒充和 Standby 可编辑声明全部失败关闭，`evidence:wp2` 报告同时确认无 Tauri、local-storage、cloud-client 或 Cloud 生产接线。真实 Boundary、Desktop Composition Root、网络、持久化和写路径仍未交付；P0-23/R0-01/R0-04/R0-10/R0-16 保持 In Progress，Standby 继续只展示固定 Fixture。

P0-22 的 Public/Admin/Jobs 入口设计已经冻结，提交 `0bead14`、`e760025` 已交付 H0～H2：Public/Admin Composition Root 与 Session/Route Surface 隔离、无业务边界 Route、framework-independent Jobs 骨架、16 行错误身份矩阵、分离 OpenAPI 文档、边界规则和 `pnpm evidence:wp4` 入口已经建立。rate-limit 边界观测只证明单进程 Fixture 分层执行固定取自 `request.ip` 的 pre-auth 地址桶和验证后的 `gd_session` 会话桶，轮换或缺失未验证 Cookie 不能逃离已耗尽的同源地址桶，并覆盖 Fixture 会话分桶、429/`Retry-After` 配对与真实墙钟窗口重置；不证明可信代理/client-IP 解析、P0-19 真实 Session、业务端点预算、per-IP/设备/账号维度、跨实例一致性或生产容量。真实账号会话与生产限流由 P0-19 承接，Audit chain、TenantJobEnvelope、逐租户 Fan-out 和周期 Job 仍未交付；P0-22/R0-09/R0-15 均保持 In Progress，Admin 业务 Route 与周期 Job 继续不注册。
