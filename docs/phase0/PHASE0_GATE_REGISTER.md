...
P0-17 的三流 Drain 设计已经冻结，提交 `38a2dad`、`3dc5277`、`416a2ca` 已交付 S1～S5：协议 Golden Corpus 固定三个独立序列域、canonical envelope、`sha256-chain-v1`、DrainProof/DrainManifest purpose 隔离和服务端富化摘要不变性；Cloud Fixture 覆盖十三步验证、逐流 Gap/水位/摘要、原子 Proof 消费/Lease 释放、Late/quarantine/drain-seal 与事务故障；client-core 覆盖纯 Drain ledger、claims builder 和 readiness。S6 evidence 扩展、S7 Rust Drain Envelope 镜像、真实签名验证和生产持久化尚未进入该技术集合，签名端口仍不能表达成功；P0-17/R0-05/R0-16 均保持 In Progress，正常 handoff 继续禁止。

P0-22 的 Public/Admin/Jobs 入口设计已经冻结，提交 `0bead14`、`e760025` 已交付 H0～H2：Public/Admin Composition Root 与 Session/Route Surface 隔离、无业务边界 Route、framework-independent Jobs 骨架、16 行错误身份矩阵、分离 OpenAPI 文档、边界规则和 `pnpm evidence:wp4` 入口已经建立。rate-limit 探针只证明单进程 Fixture 上按 `gd_session` 身份分桶、429/`Retry-After` 配对、窗口重置和缺失身份 401 路径；不证明 P0-19 真实 Session、业务端点预算、IP/设备维度、跨实例一致性或生产容量。真实账号会话与生产限流由 P0-19 承接，Audit chain、TenantJobEnvelope、逐租户 Fan-out 和周期 Job 仍未交付；P0-22/R0-09/R0-15 均保持 In Progress，Admin 业务 Route 与周期 Job 继续不注册。
