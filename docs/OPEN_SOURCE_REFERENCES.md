...
| 状态 | 当前项目含义 | 当前条目 |
| --- | --- | --- |
| Adopted | 已进入受 Lockfile 约束的构建输入，仍须通过对应 Gate | Tauri 2.11.5、Wry 0.55.1 |
| Baseline Selected | 版本/方向已由 Accepted ADR 或工程基线选定，但依赖和原生证据尚未接入 | SQLCipher 4.17.0；Rust SQLite Adapter 方向（rusqlite 版本待接入时锁定） |
