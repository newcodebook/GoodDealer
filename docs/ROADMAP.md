...
| 工作流 | 权威 | 当前基础 | 完成条件 |
| --- | --- | --- | --- |
| 账号与授权 | Cloud control plane | 身份、默认工作区、设备、持久化模块、已冻结的首切片激活/授权 grant 合同和首条 activation HTTP route | 真实登录、订阅、Lease 签发/吊销与 Desktop 验证组合。 |
...
| Desktop 业务 | Desktop + client-core + Tauri Host | UI/领域组件、本地库底座，以及由 `secure-host-core` 持有的 Host-owned 数据库身份/OS key custody | 有效授权后只通过窄 IPC 读写本地库；Cloud 失败回归通过。 |
| Provider | Secure Host | 私有 Cloudflare read service | 本地凭据录入、Host 组合、结果先写本地、真实 Provider 资格。 |
| 同步 | local Inbox/Outbox + Cloud replica | 本地复制 schema、Cloud mutations/cursors/checkpoints、显式 `workspace_replica_*` 基础和已冻结的 tenant-neutral Push/ACK/Pull/Checkpoint/恢复合同 | 实现严格白名单 push/pull materializer、冲突解决、ACK 和恢复回归。 |
| 发行 | Release | 工程与检查基础 | 最终构件、签名/公证、部署、运行证据和独立审查。 |

## 当前优先级

1. 完成授权 grant/ActiveDeviceLease 的 Cloud 签发与 Desktop Host 验签、Epoch/时间窗和工作区绑定接线。
