...
状态：Accepted Delivery Baseline / Gates Active
更新日期：2026-08-05
...
| Phase 1 | J-01 首次连接与导入；J-02 只读资产；J-05 Standby/重建；J-07 备份/迁移 | JF-01、JF-03、JF-08、JF-12 的备份内容部分；任何非 Fixture、非纯员工合成数据或外部主体数据进入前，按 D-021 完成 JF-18 部署证据，并按 D-019 完成 JF-16 的灾备、指标、告警与 Runbook 基线 |
| Phase 2 | J-02 批量计划、批准、执行和崩溃恢复；J-03 浏览器写入安全 | JF-05、JF-06；JF-17 的签名/兼容/撤销最小发布链必须在首个正式网页写连接器前完成 |
| Phase 3 | J-03 所有权验证；J-04 紧急下架 | JF-09、JF-10；按 D-016 只验收 Active 发现、Priority-0 调度和逐次批准，不实现无人值守分支 |
| Phase 4 | J-06 商业授权；J-08 Owner 后台；J-09 Support/合规/安全；J-10 生产运营 | JF-11、JF-13、JF-14、JF-16、JF-17；按 D-017～D-021 扩展并复核 Lifetime、删除/保留、SLO、支付和生产运营证据；市场、区域或复制边界变化时重新决策 |
...
每个 Phase 开始前应确认本阶段 Journey 范围、责任人、已接受决策的适用边界和证据 Finding；退出前不得遗留影响该阶段交付的 J0，并至少通过 Happy Path、权限拒绝、中断恢复和结果未知四类 E2E 场景。新发现的产品选择必须先登记并获批，不得由实现者用默认值形成新的产品承诺。
...
| 内部 Read-only Alpha | 只开放已通过平台 Transport、Host-owned Secret、设备身份、Sync Projection 和租户隔离 Gate 的只读连接；未通过的 Provider 保持 Manual/Disabled | 不承诺浏览器写入、批量副作用、紧急下架执行或商业可用性 |
...
- DeviceCredentialBindingStatus 与 HostCredentialBinding 均不进入云端，且 HostCredentialBinding/credentialRef 不进入普通 TypeScript 或 Active Workspace 的自动化泄漏测试。
- 本地加密备份导出、校验和恢复验证。
...
- 用户触发的本地加密备份导出和恢复。
- `SynchronizedBackup` 与 `EmergencyLocalSnapshot`、字段级 `BackupExportSchema`、默认关闭的平台凭据开关和永不包含清单；Emergency 恢复只形成 Candidate。
- 首次完整分页导入的 Observed/Base/Desired 初始化、快照完整性和新鲜度规则。
- 在任何非 Fixture、非纯员工合成数据，尤其任何外部主体数据，首次进入任一 GoodDealer 控制环境前按 D-021 建立 AWS 新加坡主区、悉尼加密灾备区、Development/Staging/Production 独立账号/网络/数据库/对象存储/IAM/KMS 和版本库 IaC。环境被命名为 internal pilot、beta 或其他名称不能绕过该谓词。
- 上述受控环境必须在接收数据前启用 Multi-AZ、PITR、加密灾备和删除传播，形成 D-019 的 RPO 15 分钟/RTO 4 小时、事故 Owner、关键 SLI、告警阈值与 Runbook，并完成至少一次告警响应和恢复演练；公开 Cloud SLO 为 99.5%，首期没有赔付型 SLA。

退出条件：已登录且授权有效的用户可以在本地统一查看首批平台状态；Offline Device Lease 与 Entitlement 均有效时断网仍可查询。两类备份的名称、Manifest、恢复语义和失败关闭行为通过 J-07 场景。任何非 Fixture、非纯员工合成数据或外部主体数据进入任一 GoodDealer 控制环境前，JF-18 的区域、环境、KMS/IaC、PITR、RPO/RTO、Owner/Runbook 与恢复演练证据必须先完成；没有这些证据时只能使用内部 Fixture/纯员工合成数据，Alpha/Beta/Pilot/Production 名称不能放宽该边界。
...
- 旧 Epoch LateExecutionEvent 验证、追加审计、`outcome_unknown` 延续、StaleChangeProposal 服务器字段伪造拒绝和 Cloud 生成 StaleDeviceCandidate 的分流测试。
...
退出条件：VerificationAttempt、DNS 条件写和 Operations 结果交接可在重复、超时和崩溃后幂等恢复；同名 RRset 不被整组覆盖，挑战值不进入普通 TypeScript、Outbox 或 Cloud；权威与递归证据能够区分写入确认和传播完成。Active 应用在用户主动刷新或既有已批准业务读取中形成 SaleSignal、人工批准紧急处置与逐次授权证据的基线已冻结；后台周期轮询、OS 后台唤醒、Cloud Relay、Standing Policy 和无人值守执行均不属于首版。外部修改不会被静默覆盖，J-03/J-04 的权限拒绝、中断恢复和 `outcome_unknown` 场景通过。
...
- 生产 Admin Web、Owner Passkey、Scope、管理 Port、跨账号访问理由/有效 `AdminPurposeRef`、重新认证与 Staff 审计；多人角色与审批留到增加 Staff 后。
- 外部 Helpdesk 接入及内部可信 SupportCaseReference；DataRightsRequest/SecurityIncident 仍由内部模块拥有。
- License/订单/设备/同步/合规/版本通道运营功能，以及 Public/Admin Route 隔离渗透测试。
- Sunset Signing Key 隔离演练、最终本地延续版本和永久离线凭证流程。
- Windows 安装包与 macOS 公证。
- 崩溃恢复、迁移和备份恢复测试。

退出条件：支付事件按 D-020 在重复、乱序、退款和拒付下可确定性重建 Entitlement，账号恢复、License、设备撤销和续费恢复通过端到端测试；Windows 签名安装包与 macOS 公证制品可由锁定流水线重建并验证更新、回滚和撤销。D-017～D-021 的 Lifetime 演练、删除/保留、SLO、支付、区域和跨境政策完成生产级证据及 Privacy/Legal 复核；市场、区域或复制边界变化时重新审理。Public/Admin/Jobs、租户隔离、唯一 Owner 恢复演练、Owner 高风险动作、外部 Helpdesk SupportCaseReference 和数据权利流程通过权限拒绝与审计复核。未通过平台安全或运营 Gate 的连接器能力保持 Disabled/Manual，不以商业发布日期豁免。
...
- 移动端切换为 Active 后的正式批准、平台刷新、少量编辑和经批准的平台紧急下架执行；首版不提供无人值守能力。
- 后台任务受限时的安全暂停和恢复。
- 文件分享和生物识别解锁。
