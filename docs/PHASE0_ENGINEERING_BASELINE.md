...
- 两个 Workflow 均支持 PR、`main` Push 和人工重跑，不使用路径过滤。相同 PR 的旧运行会被取消；`main` 与人工运行保留完整执行，避免 Gate 证据被后续 Push 静默中断。Workflow 仅授予 `contents: read` 与用于解析精确 Job URL 的 `actions: read`，Checkout 不持久化 GitHub 凭据；Action 引用必须固定到完整 Commit SHA。
...
Evidence Collector 在 Windows 通过 `.cmd` shim 启动 pnpm，并在 Desktop Build 成功产出 `frontendDist` 后才执行完整 Rust Workspace Gate。Tauri 按 Windows 宿主派生的 `windows-schema.json` 不属于仓库输入；Capability 的可移植契约以受版本控制的 `desktop-schema.json`、Capability JSON、Permission TOML 和结构门禁为准。
